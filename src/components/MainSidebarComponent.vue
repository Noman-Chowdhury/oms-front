...
const props = defineProps(['isCollapsed', 'isTwoColumnMenu', 'isSidebarMini', 'isSubMenuCollapsed', 'closeMainLeftSidebar', 'isAdmin'])
...
import {userSidebarMenus} from "@/composable/sidebarMenu"
  ...
  >
    <div v-click-outside="closeMainLeftSidebar" class="main-menu">
      <div class="scrollable sidebar-menu" :id="horizontalMenuEnabled ? 'accordionExample' : 'testAccordionExample'">
        <li v-for="(sidebar, index) in sidebarMenus" class="sidebar-item"
            :class="[horizontalMenuEnabled ? 'dropdown': '']">
          <div v-if="!sidebar.menu_name_disable">
            <template v-if="horizontalMenuEnabled">
              <a role="button" class="sidebar-link-group-title has-sub" :id="'parentDropdownMenu'+index"
                 :data-bs-toggle="[horizontalMenuEnabled ? 'dropdown' : '']" data-bs-auto-close="outside"
                 aria-expanded="false">
                {{ $t(sidebar.menu_name) }}
              </a>
            </template>
            <template v-else>
              <a role="button" class="sidebar-link-group-title has-sub" data-bs-toggle="collapse"
                 :href="`#collapseExample-${index}`" aria-expanded="false" :aria-controls="'collapseExample-'+index">
                {{ $t(sidebar.menu_name) }}
              </a>
            </template>
          </div>
          <template v-if="sidebar.menus">
            <ul class="sidebar-link-group" :class="[horizontalMenuEnabled ? 'dropdown-menu' : 'show']"
                :aria-labelledby="[horizontalMenuEnabled ? 'parentDropdownMenu'+index  : '']"
                :id="[horizontalMenuEnabled ? 'AppDropDownId'+index : `collapseExample-${index}`]"
                data-bs-parent="#testAccordionExample">
              <li v-for="(menu, mIndex) in sidebar.menus" class="sidebar-dropdown-item">
                <div v-if="!menu.permissions || (menu.permissions && userInfoStore.hasPermission(menu.permissions))">
                  <template v-if="menu.link_name">
                    <router-link :to="{ name: `${menu.link_name}` }" class="sidebar-link"
                                 :class="{active : currentRoute === menu.link_name }">
                      <span class="nav-icon"><i :class="menu.icon"></i></span> <span class="sidebar-txt">{{
                        $t(menu.name)
                      }}</span>
                    </router-link>
                  </template>
                  <template v-if="menu.sub_menus">
                    <template v-if="horizontalMenuEnabled">
                      <a role="button" class="sidebar-link has-sub" :id="'parentSubDropdownMenu'+index"
                         data-bs-toggle="dropdown" data-bs-auto-close="outside" aria-expanded="false">
                        <span v-if="menu.icon" class="nav-icon"><i :class="menu.icon"></i></span> <span
                          class="sidebar-txt">{{ $t(menu.name) }}</span>
                      </a>
                    </template>
                    <template v-else>
                      <a role="button" class="sidebar-link has-sub" data-bs-toggle="collapse"
                         :href="`#subCollapseMenu-${mIndex}-${index}`" aria-expanded="false"
                         :aria-controls="`subCollapseMenu-${mIndex}-${index}`" @click="findMenuByName(menu.name)">
                        <span v-if="menu.icon" class="nav-icon"><i :class="menu.icon"></i></span> <span
                          class="sidebar-txt">{{ $t(menu.name) }}</span>
                      </a>
                    </template>
                ...
                <ul v-click-outside="closeToggleMenu" class="sidebar-dropdown-menu collapse"
                    :class="[horizontalMenuEnabled ? 'dropdown-menu' : '', openedMenu === menu.name ? 'd-block' : '', (layoutPosition === 'twoColumn' && index === 1) ? 'show' : '']"
                    :id="[horizontalMenuEnabled ? `subDropDownId-${mIndex}-${index}` : `subCollapseMenu-${mIndex}-${index}`]"
                    :aria-labelledby="[horizontalMenuEnabled ? `parentSubDropdownMenu-${mIndex}-${index}`  : '']">
                  <li v-for="(sub_menu, sIndex) in menu.sub_menus" class="sidebar-dropdown-item">
                    <div v-if="!sub_menu.permissions || (sub_menu.permissions && userInfoStore.hasPermission(sub_menu.permissions))">
                    <template v-if="sub_menu.link_name">
                      <router-link :to="{ name: `${sub_menu.link_name}` }" class="sidebar-link"
                                   :class="{active : currentRoute === sub_menu.link_name }">
                        <span v-if="sub_menu.icon" class="nav-icon"><i :class="sub_menu.icon"></i></span> <span
                          :class="{'sidebar-txt': (currentNavbarSize !== 'small' && layoutPosition !== 'twoColumn')}">{{
                          sub_menu.name
                        }}</span>
                      </router-link>
                    </template>
                    <template v-else>
                      <router-link to="#" class="sidebar-link" :class="{active : currentRoute === sub_menu.link_name }">
                        <span v-if="sub_menu.icon" class="nav-icon"><i v-if="sub_menu.icon" :class="sub_menu.icon"></i></span>
                    ...
                    <template v-if="sub_menu.sub_menus">
                      <template v-if="horizontalMenuEnabled">
                        <a role="button" class="sidebar-link has-sub" :id="'parentSubDropdownMenu'+index"
                           data-bs-toggle="dropdown" data-bs-auto-close="outside" aria-expanded="false"
                           @click="findMenuByName(sub_menu.name)">
                          <span v-if="sub_menu.icon" class="nav-icon"><i :class="sub_menu.icon"></i></span> <span
                            class="sidebar-txt">{{ $t(sub_menu.name) }}</span>
                        </a>
                      </template>
                      <template v-else>
                        <a role="button" class="sidebar-link has-sub" data-bs-toggle="collapse"
                           :href="`#subCollapseMenu-${mIndex}-${index}-${sIndex}`" aria-expanded="false"
                           :aria-controls="`subCollapseMenu-${mIndex}-${index}-${sIndex}`"
                           @click="findMenuByName(sub_menu.name)">
